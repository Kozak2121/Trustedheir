...
angular
    .module('trustedheir')
    .controller('TrusteeDigitalAssetsCtrl', ['$stateParams', '$scope', '$state', '$window', '$document', '$location', '$timeout', '$compile', ($stateParams, $scope, $state, $window, $document, $location, $timeout, $compile) => {
        ...
        if (currentUser) {
            Meteor.call('getAssetsByTrusteeId', currentUser._id, initialQuery, (err, cb) => {
                if (err) {
        ...
        self.getMoreItems = () => {
            if (self.assets && self.assets.length > 0) {
                ...
                Meteor.call('getAssetsByTrusteeId', currentUser._id, query, (err, cb) => {
                    if (err) {
                        return toastr.error(err.message, 'ERROR');
                    }
        ...
        self.generateHiddenCategories = () => {
            ...
            self.hiddenCategories.forEach(category => {
                let tplCrop = `<li ng-class="{active: tda.category==='${category}'}" ng-click="tda.selectCategory($event)"> ${category} </li>`;
        ...
        self.digitalAssetViewDetails = assetId => $location.path(`/trustee/digital-asset/post-passing/${assetId}`);
